...
class DH_Luxury_Theme {
	...
	public function after_setup_theme(){
		load_theme_textdomain( 'luxury-wp', get_template_directory() . '/languages' );
		...
		$nav_menus['top'] 		= esc_html__( 'Top Menu', 'luxury-wp' );
		$nav_menus['primary'] 	= esc_html__( 'Primary Menu', 'luxury-wp' );
		$nav_menus['category'] = esc_html__( 'Categories Menu', 'luxury-wp' );
		$nav_menus['mobile'] = esc_html__( 'Mobile Menu', 'luxury-wp' );
		$nav_menus['sidebar'] = esc_html__( 'Sidebar Menu', 'luxury-wp' );
		$nav_menus['footer'] = esc_html__( 'Footer Menu', 'luxury-wp' );
	...
	protected function _includes(){
		...
		if ( class_exists('TGM_Plugin_Activation')) {
			include_once (get_template_directory() . '/includes/plugins/plugin-activation.php');
		}
	...
	public function enqueue_theme_styles(){
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'luxury-wp' ) ) {
			wp_enqueue_style('dh-theme-google-font','//fonts.googleapis.com/css?family=Poppins:300,500,600&subset=latin,latin-ext');
		}
		...
		if(is_rtl()){
			wp_enqueue_style($main_css_id.'-rtl',get_template_directory_uri().'/assets/css/rtl' . $suffix . '.css',array($main_css_id),DH_THEME_VERSION);
		}
	...
	public function enqueue_theme_script(){
		...
		if(defined('WOOCOMMERCE_VERSION') && dh_get_theme_option('woo-lazyload',1))
		...
		$dhL10n = array(
			...
			'ajax_finishedMsg'=>esc_attr__('All posts displayed','luxury-wp'),
			'ajax_msgText'=>esc_attr__('Loading the next set of posts...','luxury-wp'),
			...
			'loadingmessage'=>esc_attr__('Sending info, please wait...','luxury-wp'),
		);
	...
	public function register_sidebar() {
		...
		register_sidebar( 
			array(  // 1
				'name' => esc_html__( 'Main Sidebar', 'luxury-wp' ), 
				'description' => esc_html__( 'This is main sidebar', 'luxury-wp' ), 
				'id' => 'sidebar-main', 
		...
		register_sidebar( 
			array(  // 1
				'name' => esc_html__( 'Shop Sidebar', 'luxury-wp' ), 
				'description' => esc_html__( 'This sidebar use for Woocommerce page', 'luxury-wp' ), 
				'id' => 'sidebar-shop', 
				'before_widget' => '<div id="%1$s" class="widget %2$s">', 
				'after_widget' => '</div>', 
				'before_title' => '<h4 class="widget-title"><span>', 
		...
		register_sidebar(
			array(  // 1
				'name' => esc_html__( 'Shop Filter Sidebar', 'luxury-wp' ),
				'description' => esc_html__( 'This sidebar use for Woocommerce Filter page', 'luxury-wp' ),
		...
		register_sidebar(
			array(  // 1
				'name' => esc_html__( 'Off-canvas Sidebar', 'luxury-wp' ),
				'description' => esc_html__( 'This sidebar use for Offcanvas Sidebar', 'luxury-wp' ),
		...
		for ( $i = 1; $i <= 5; $i++ ) :
			register_sidebar( 
				array( 
					'name' => esc_html__( 'Footer Sidebar #', 'luxury-wp' ) . $i, 
					'id' => 'sidebar-footer-' . $i, 
					'before_widget' => '<div id="%1$s" class="widget %2$s">', 
					'after_widget' => '</div>', 
...
$dh_luxury_theme = new DH_Luxury_Theme();
