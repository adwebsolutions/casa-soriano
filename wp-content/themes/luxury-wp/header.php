<?php 
	$navbar_position = dh_get_post_meta('navbar-position',get_the_ID(),'top');
...
<div id="preloader">
	...
	<div class="preloader__progress">
		<svg width="60px" height="60px" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
			...
			<path id='preloader__progress-circle' fill="none" stroke="<?php echo dh_format_color(dh_get_theme_option('brand-primary','#9fce4e'))?>" stroke-width="4" stroke-linecap="round" stroke-dashoffset="192.61" stroke-dasharray="192.61 192.61" d="M40,10C57.351,10,71,23.649,71,40.5S57.351,71,40.5,71 S10,57.351,10,40.5S23.649,10,40.5,10z"/>
		</svg>
	</div>
</div>
<?php endif;?>
<a class="sr-only sr-only-focusable" href="#main"><?php esc_html_e('Skip to main content','luxury-wp') ?></a>
<?php
if('overlay' !== $header_style )
...
<?php if(!dh_get_post_meta('hide_header',0)):?>
		...
		<?php elseif ($page_heading === 'default'):?>
			<?php if(dh_get_theme_option('breadcrumb',1) ):?>
			<div<?php echo (!empty($heading_menu_anchor) ? ' id ="'.esc_attr($heading_menu_anchor).'"': '')?> class="heading-container heading-standar-wrap">
