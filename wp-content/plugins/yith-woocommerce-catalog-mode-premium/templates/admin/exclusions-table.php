			...
			$sections   = array(
				...
				'categories' => array(
					...
					'options' => array(
						...
						'count_table'      => '( SELECT a.* FROM ' . $wpdb->prefix . 'terms a INNER JOIN ' . $wpdb->prefix . 'term_taxonomy b ON a.term_id = b.term_id INNER JOIN ' . $wpdb->prefix . $this->get_table_id_wc_prefix() . 'termmeta c ON c.' . $this->get_table_id_wc_prefix() . 'term_id = a.term_id WHERE b.taxonomy = "product_cat" AND ( c.meta_key = "_ywctm_exclude_catalog_mode' . $this->get_vendor_id() . '" OR c.meta_key = "_ywctm_exclude_hide_price' . $this->get_vendor_id() . '" ) AND c.meta_value = "yes" GROUP BY a.term_id ) AS a',
						'count_where'      => '',
						'key_column'       => 'ID',
						'view_columns'     => array(
							'cb'          => '<input type="checkbox" />',
								...
								'function_delete' => function () {
									...
									if ( ! empty( $ids ) ) {
										$wpdb->query( "UPDATE {$wpdb->prefix}{$this->get_table_id_wc_prefix()}termmeta
                                           SET meta_value = 'no'
                                           WHERE ( meta_key = '_ywctm_exclude_catalog_mode{$this->get_vendor_id()}' OR meta_key = '_ywctm_exclude_hide_price{$this->get_vendor_id()}' ) AND {$this->get_table_id_wc_prefix()}term_id IN ( $ids )"
										);
									}
						...
						case 'categories':
							...
							foreach ( $category_ids as $category_id ) {
								...
								if ( YITH_WCTM()->is_wc_lower_2_6 ) {
									...
									update_woocommerce_term_meta( $category_id, '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), $add_to_cart );
									update_woocommerce_term_meta( $category_id, '_ywctm_exclude_hide_price' . $this->get_vendor_id(), $price );
						...
						case 'tags':
							...
							foreach ( $tag_ids as $tag_id ) {
								...
								if ( YITH_WCTM()->is_wc_lower_2_6 ) {
									...
									update_woocommerce_term_meta( $tag_id, '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), $add_to_cart );
									update_woocommerce_term_meta( $tag_id, '_ywctm_exclude_hide_price' . $this->get_vendor_id(), $price );
						...
						default:
							...
							if ( ! empty( $product_ids ) ) {
								...
								foreach ( $product_ids as $product_id ) {
									...
									update_post_meta( $product_id, '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), $add_to_cart );
									update_post_meta( $product_id, '_ywctm_exclude_hide_price' . $this->get_vendor_id(), $price );
				...
				switch ( $current_section ) {
					...
					case'categories':
						...
						if ( YITH_WCTM()->is_wc_lower_2_6 ) {
							...
							$item = array(
								'ID'          => $_GET['id'],
								'add_to_cart' => get_woocommerce_term_meta( $_GET['id'], '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), true ),
								'price'       => get_woocommerce_term_meta( $_GET['id'], '_ywctm_exclude_hide_price' . $this->get_vendor_id(), true ),
							);
					...
					default:
						...
						$item = array(
							'ID'          => $_GET['id'],
							'add_to_cart' => get_post_meta( $_GET['id'], '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), true ),
							'price'       => get_post_meta( $_GET['id'], '_ywctm_exclude_hide_price' . $this->get_vendor_id(), true ),
						);
