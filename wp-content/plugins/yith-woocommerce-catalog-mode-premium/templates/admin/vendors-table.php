	...
	class YWCTM_Vendors_Table {
		...
		public function output() {
			...
			$table->options = array(
				...
				'count_table'      => '( SELECT a.* FROM ' . $wpdb->prefix . 'terms a INNER JOIN ' . $wpdb->prefix . 'term_taxonomy b ON a.term_id = b.term_id INNER JOIN ' . $wpdb->prefix . $this->get_table_id_wc_prefix() . 'termmeta c ON c.' . $this->get_table_id_wc_prefix() . 'term_id = a.term_id WHERE b.taxonomy = "yith_shop_vendor" AND ( c.meta_key = "_ywctm_vendor_override_exclusion" ) AND c.meta_value = "yes" GROUP BY a.term_id ) AS a',
				'count_where'      => '',
				'key_column'       => 'ID',
				'view_columns'     => array(
						...
						'function_delete' => function () {
							...
							if ( ! empty( $ids ) ) {
								$wpdb->query( "UPDATE {$wpdb->prefix}{$this->get_table_id_wc_prefix()}termmeta
                                           SET meta_value = 'no'
                                           WHERE ( meta_key = '_ywctm_vendor_override_exclusion' ) AND {$this->get_table_id_wc_prefix()}term_id IN ( $ids )"
								);
							}
				...
				if ( $item_valid !== true ) {
					...
					foreach ( $vendor_ids as $vendor_id ) {
						...
						if ( YITH_WCTM()->is_wc_lower_2_6 ) {
							...
							update_woocommerce_term_meta( $vendor_id, '_ywctm_vendor_override_exclusion', $exclusion );
			...
			if ( isset( $_GET['id'] ) && ! empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
				...
				if ( YITH_WCTM()->is_wc_lower_2_6 ) {
					...
					$item = array(
						'ID'      => $_GET['id'],
						'exclude' => get_woocommerce_term_meta( $_GET['id'], '_ywctm_vendor_override_exclusion', true )
					);
						...
						<table class="form-table">
							...
							<tr valign="top">
								...
								<td class="forminp">
									...
									<input type="hidden" class="wc-product-search" id="vendor_ids" name="vendor_ids"
									       data-placeholder="<?php _e( 'Search for a vendor&hellip;', 'yith-woocommerce-catalog-mode' ); ?>"
