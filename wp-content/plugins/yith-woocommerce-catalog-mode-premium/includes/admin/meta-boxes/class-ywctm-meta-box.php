...
if ( ! class_exists( 'YWCTM_Meta_Box' ) ) {
	...
	class YWCTM_Meta_Box {
		...
		public function save() {
			...
			update_post_meta( $post->ID, '_ywctm_exclude_catalog_mode', $catalog_mode );
			update_post_meta( $post->ID, '_ywctm_exclude_hide_price', $hide_price );
			...
			if ( get_option( 'ywctm_custom_button' ) == 'yes' || get_option( 'ywctm_custom_button_loop' ) == 'yes' ) {
				...
				update_post_meta( $post->ID, '_ywctm_custom_url_enabled', $enable_url );
				update_post_meta( $post->ID, '_ywctm_exclude_button', $exclude );
				update_post_meta( $post->ID, '_ywctm_custom_url_protocol', $protocol );
				update_post_meta( $post->ID, '_ywctm_custom_url_link', $link );
	...
	/**
	 * Unique access to instance of YWCTM_Meta_Box class
	 *
	 * @return \YWCTM_Meta_Box
	 */
	function YWCTM_Meta_Box() {

		return YWCTM_Meta_Box::get_instance();
