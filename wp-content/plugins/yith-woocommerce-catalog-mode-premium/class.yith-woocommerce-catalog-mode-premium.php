...
if ( ! class_exists( 'YITH_WC_Catalog_Mode_Premium' ) ) {
	...
	class YITH_WC_Catalog_Mode_Premium extends YITH_WC_Catalog_Mode {
		...
		public function write_taxonomy_options( $taxonomy ) {
			...
			if ( get_option( 'ywctm_hide_add_to_cart_single' ) == 'yes' || get_option( 'ywctm_hide_add_to_cart_loop' ) == 'yes' || get_option( 'ywctm_hide_price' ) == 'yes' ) {
				...
				if ( $this->is_wc_lower_2_6 ) {
					...
					$add_to_cart = get_woocommerce_term_meta( $taxonomy->term_id, '_ywctm_exclude_catalog_mode', true ) == 'yes' ? 'checked' : '';
					$price       = get_woocommerce_term_meta( $taxonomy->term_id, '_ywctm_exclude_hide_price', true ) == 'yes' ? 'checked' : '';
				...
				<?php if ( get_option( 'ywctm_custom_button' ) == 'yes' || get_option( 'ywctm_custom_button_loop' ) == 'yes' ) : ?>
					...
					if ( $this->is_wc_lower_2_6 ) {
						...
						$button    = get_woocommerce_term_meta( $taxonomy->term_id, '_ywctm_custom_url_enabled', true ) == 'yes' ? 'checked' : '';
						$no_button = get_woocommerce_term_meta( $taxonomy->term_id, '_ywctm_exclude_button', true ) == 'yes' ? 'checked' : '';
						$protocol  = get_woocommerce_term_meta( $taxonomy->term_id, '_ywctm_custom_url_protocol', true );
						$link      = get_woocommerce_term_meta( $taxonomy->term_id, '_ywctm_custom_url_link', true );
					...
					<tr class="form-field">
						<th>
							<label for="_ywctm_exclude_button"><?php _e( 'Exclude products from custom button', 'yith-woocommerce-catalog-mode' ); ?></label>
						</th>
						<td>
							<input id="_ywctm_exclude_button" name="_ywctm_exclude_button" type="checkbox" <?php echo $no_button; ?> />
						</td>
					</tr>
					<tr class="form-field">
						<th>
						...
						<td>
							<input id="_ywctm_custom_url_enabled" name="_ywctm_custom_url_enabled" type="checkbox" <?php echo $button; ?> />
						</td>
					</tr>
					<tr class="form-field">
						<th>
						...
						<td>
							<select id="_ywctm_custom_url_protocol" name="_ywctm_custom_url_protocol">
								<option value="generic" <?php selected( $protocol, 'generic' ); ?>><?php _e( 'Generic URL', 'yith-woocommerce-catalog-mode' ); ?></option>
								<option value="mailto" <?php selected( $protocol, 'mailto' ); ?>><?php _e( 'E-mail address', 'yith-woocommerce-catalog-mode' ); ?></option>
								<option value="tel" <?php selected( $protocol, 'tel' ); ?>><?php _e( 'Phone number', 'yith-woocommerce-catalog-mode' ); ?></option>
								<option value="skype" <?php selected( $protocol, 'skype' ); ?>><?php _e( 'Skype contact', 'yith-woocommerce-catalog-mode' ); ?></option>
							</select>
						</td>
					</tr>
					<tr class="form-field">
		...
		public function save_taxonomy_options( $taxonomy_id ) {
			...
			if ( get_option( 'ywctm_hide_add_to_cart_single' ) == 'yes' || get_option( 'ywctm_hide_add_to_cart_loop' ) == 'yes' || get_option( 'ywctm_hide_price' ) == 'yes' ) {
				...
				if ( $this->is_wc_lower_2_6 ) {
					...
					update_woocommerce_term_meta( $taxonomy_id, '_ywctm_exclude_catalog_mode', $add_to_cart );
					update_woocommerce_term_meta( $taxonomy_id, '_ywctm_exclude_hide_price', $price );
				...
				if ( get_option( 'ywctm_custom_button' ) == 'yes' || get_option( 'ywctm_custom_button_loop' ) == 'yes' ) {
					...
					if ( $this->is_wc_lower_2_6 ) {
						...
						update_woocommerce_term_meta( $taxonomy_id, '_ywctm_custom_url_enabled', $button );
						update_woocommerce_term_meta( $taxonomy_id, '_ywctm_exclude_button', $no_button );
						update_woocommerce_term_meta( $taxonomy_id, '_ywctm_custom_url_protocol', $protocol );
						update_woocommerce_term_meta( $taxonomy_id, '_ywctm_custom_url_link', $link );
		...
		public function country_check( $post_id ) {
			...
			if ( in_array( $this->_user_geolocation, explode( ',', $countries ) ) ) {
				$result = true;
			}
		...
		public function get_exclusion( $value, $post_id, $option ) {
			...
			if ( apply_filters( 'ywctm_get_vendor_postmeta', get_post_meta( $post_id, $option, true ), $post_id, $option ) == 'yes' ) {
			...
			foreach ( $product_cats as $cat_id ) {
				...
				if ( $this->is_wc_lower_2_6 ) {
					...
					if ( apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $cat_id, $option, true ), $post_id, $cat_id, $option ) == 'yes' ) {
						return 'yes';
					}
			...
			foreach ( $product_tags as $tag_id ) {
				...
				if ( $this->is_wc_lower_2_6 ) {
					...
					if ( apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $tag_id, $option, true ), $post_id, $tag_id, $option ) == 'yes' ) {
						return 'yes';
					}
		...
		public function add_inquiry_form_tab( $tabs = array() ) {
			...
			if ( $form_type != 'none' && ( function_exists( 'YIT_Contact_Form' ) || function_exists( 'wpcf7_contact_form' ) || function_exists( 'gravity_form' ) ) ) {

				if ( class_exists( 'SitePress' ) ) {

					$contact_form_7   = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_contact_form_7_id_wpml' ), $post->ID, 'ywctm_inquiry_contact_form_7_id_wpml' );
					$gravity_forms    = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_gravity_forms_id_wpml' ), $post->ID, 'ywctm_inquiry_gravity_forms_id_wpml' );
					$yit_contact_form = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_yit_contact_form_id_wpml' ), $post->ID, 'ywctm_inquiry_yit_contact_form_id_wpml' );
					...
					$contact_form_7   = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_contact_form_7_id' ), $post->ID, 'ywctm_inquiry_contact_form_7_id' );
					$gravity_forms    = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_gravity_forms_id' ), $post->ID, 'ywctm_inquiry_gravity_forms_id' );
					$yit_contact_form = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_yit_contact_form_id' ), $post->ID, 'ywctm_inquiry_yit_contact_form_id' );
		...
		public function get_custom_button_template() {
			...
			if ( apply_filters( 'ywctm_get_vendor_postmeta', get_post_meta( $post->ID, '_ywctm_exclude_button', true ), $post->ID, '_ywctm_exclude_button' ) == 'yes' ) {
				return;
			}

			if ( apply_filters( 'ywctm_get_vendor_postmeta', get_post_meta( $post->ID, '_ywctm_custom_url_enabled', true ), $post->ID, '_ywctm_custom_url_enabled' ) == 'yes' ) {

				$protocol = apply_filters( 'ywctm_get_vendor_postmeta', get_post_meta( $post->ID, '_ywctm_custom_url_protocol', true ), $post->ID, '_ywctm_custom_url_protocol' );
				$link     = apply_filters( 'ywctm_get_vendor_postmeta', get_post_meta( $post->ID, '_ywctm_custom_url_link', true ), $post->ID, '_ywctm_custom_url_link' );
				...
				foreach ( $product_cats as $cat_id ) {
					...
					if ( $this->is_wc_lower_2_6 ) {
						...
						if ( apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $cat_id, '_ywctm_exclude_button', true ), $post->ID, $cat_id, '_ywctm_exclude_button' ) == 'yes' ) {
							return;
						}

						if ( apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $cat_id, '_ywctm_custom_url_enabled', true ), $post->ID, $cat_id, '_ywctm_custom_url_enabled' ) == 'yes' ) {

							$protocol = apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $cat_id, '_ywctm_custom_url_protocol', true ), $post->ID, $cat_id, '_ywctm_custom_url_protocol' );
							$link     = apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $cat_id, '_ywctm_custom_url_link', true ), $post->ID, $cat_id, '_ywctm_custom_url_link' );

						}

					} else {

						if ( apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $cat_id, '_ywctm_exclude_button', true ), $post->ID, $cat_id, '_ywctm_exclude_button' ) == 'yes' ) {
							return;
						}

						if ( apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $cat_id, '_ywctm_custom_url_enabled', true ), $post->ID, $cat_id, '_ywctm_custom_url_enabled' ) == 'yes' ) {

							$protocol = apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $cat_id, '_ywctm_custom_url_protocol', true ), $post->ID, $cat_id, '_ywctm_custom_url_protocol' );
							$link     = apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $cat_id, '_ywctm_custom_url_link', true ), $post->ID, $cat_id, '_ywctm_custom_url_link' );
				...
				if ( $protocol == '' ) {
					...
					foreach ( $product_tags as $tag_id ) {
						...
						if ( $this->is_wc_lower_2_6 ) {
							...
							if ( apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $tag_id, '_ywctm_exclude_button', true ), $post->ID, $tag_id, '_ywctm_exclude_button' ) == 'yes' ) {
								return;
							}

							if ( apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $tag_id, '_ywctm_custom_url_enabled', true ), $post->ID, $tag_id, '_ywctm_custom_url_enabled' ) == 'yes' ) {

								$protocol = apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $tag_id, '_ywctm_custom_url_protocol', true ), $post->ID, $tag_id, '_ywctm_custom_url_protocol' );
								$link     = apply_filters( 'ywctm_get_vendor_termmeta', get_woocommerce_term_meta( $tag_id, '_ywctm_custom_url_link', true ), $post->ID, $tag_id, '_ywctm_custom_url_link' );

							}

						} else {

							if ( apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $tag_id, '_ywctm_exclude_button', true ), $post->ID, $tag_id, '_ywctm_exclude_button' ) == 'yes' ) {
								return;
							}

							if ( apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $tag_id, '_ywctm_custom_url_enabled', true ), $post->ID, $tag_id, '_ywctm_custom_url_enabled' ) == 'yes' ) {

								$protocol = apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $tag_id, '_ywctm_custom_url_protocol', true ), $post->ID, $tag_id, '_ywctm_custom_url_protocol' );
								$link     = apply_filters( 'ywctm_get_vendor_termmeta', get_term_meta( $tag_id, '_ywctm_custom_url_link', true ), $post->ID, $tag_id, '_ywctm_custom_url_link' );
				...
				if ( $protocol == '' ) {
					...
					$link     = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_button_url' ), $post->ID, 'ywctm_button_url' );
		...
		public function check_product_price_single( $priority = true, $product_id = false ) {
			...
			if ( get_option( 'ywctm_enable_plugin' ) == 'yes' && $this->check_user_admin_enable() && $hide_price == 'yes' ) {
				...
				if ( $this->apply_catalog_mode( $post_id ) ) {
					...
					$alternative_text = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_exclude_price_alternative_text' ), $post_id, 'ywctm_exclude_price_alternative_text' );
		...
		public function show_product_price( $price, $product ) {
			...
			$hide_price = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_hide_price' ), $product->id, 'ywctm_hide_price' );

			if ( $hide_price == 'yes' && $this->apply_catalog_mode( $product->id ) ) {
		...
		public function set_price_label( $price, $product ) {
			...
			$alternative_text = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_exclude_price_alternative_text' ), $product->id, 'ywctm_exclude_price_alternative_text' );
			$enable_exclusion = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_exclude_hide_price' ), $product->id, 'ywctm_exclude_hide_price' );
			$exclude_catalog  = apply_filters( 'ywctm_get_exclusion', get_post_meta( $product->id, '_ywctm_exclude_hide_price', true ), $product->id, '_ywctm_exclude_hide_price' );

			$remove = ( $enable_exclusion != 'yes' ? true : ( $exclude_catalog != 'yes' ? true : false ) );

			$reverse_criteria = apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_exclude_hide_price_reverse' ), $product->id, 'ywctm_exclude_hide_price_reverse' );
